...
router.get('/', async (ctx, next) => {
  return ctx.response.body = 'server response';
});
...
router.post('/api/services', async (ctx, next) => {
  const jsonParseData = JSON.parse(ctx.request.body);
  const id = jsonParseData.id;
  ...
  if (+id !== 0) {
    ...
    services[index] = jsonParseData;
    services[index].id = +jsonParseData.id;
